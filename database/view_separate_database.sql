...
-- Usage: mysql -u root -p < view_separate_database.sql
...
SELECT 
    ...
    CONCAT('₹', FORMAT(TotalAmount, 2)) AS 'Total Amount',
    Address AS 'Delivery Address',
    Status,
    CreatedAt AS 'Order Date'
FROM orders
ORDER BY CreatedAt DESC;
...
SELECT 
    ...
    oi.ItemName,
    oi.Quantity,
    CONCAT('₹', FORMAT(oi.Price, 2)) AS 'Price',
    oi.CreatedAt AS 'Added On'
FROM order_items oi
